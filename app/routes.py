...
from app import app
from app.forms import LoginForm, RegistrationForm, EditProfileForm, QuizForm, ForgotPasswordForm, ResetPasswordForm
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user is None or not user.check_password(form.password.data):
            flash('Nama pengguna atau kata sandi tidak valid')
...
def user(username):
    ...
    if user is None:
        flash('Pengguna {} tidak ditemukan.'.format(username))
        return redirect(url_for('index'))
...
def edit_profile():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Perubahan Anda telah disimpan.')
...
def forgot_password():
    ...
    if form.validate_on_submit():
        ...
        flash(f'Kode recovery Anda adalah: {user.email}')
        return redirect(url_for('reset_password'))
    return render_template('forgot_password.html', title='Lupa Kata Sandi', form=form)
...
def reset_password():
    ...
    if form.validate_on_submit():
        user = db.session.scalar(
            sa.select(User).where(User.email == form.recovery_code.data))
        if user is None:
            flash('Kode recovery tidak valid')
            return redirect(url_for('reset_password'))
        user.set_password(form.password.data)
        db.session.commit()
        flash('Kata sandi Anda telah direset')
        return redirect(url_for('login'))
    return render_template('reset_password.html', title='Reset Kata Sandi', form=form)
